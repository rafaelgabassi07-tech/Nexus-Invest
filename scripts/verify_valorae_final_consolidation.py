...
checks = {
    'Proxy+ não dispara endpoints avançados na abertura do app': 'refreshProxyCapabilities(force = false)' not in vm and 'Proxy+ é carregado sob demanda' in vm,
    ...
    'Proxy+ removido da barra inferior, mantendo diagnóstico em Configurações': 'ProxyToolsScreen(' not in main and 'Proxy+' not in main and 'activePage == 5' not in main,
