...
/**
 * ATUALIZADO v16 — ASSET_PRESETS with full support for ACAO/FII/BDR/ETF/STOCK.
 * URLs based on documentation from investidor10_scraping_guide.md.
 */
const ASSET_PRESETS: Record<ExtendedAssetType, {
  ...
  /** NOVO v16 — Stocks foreign listed on Investidor10. */
  ...
  const p = (async () => {
    ...
    try {
      ...
      if (res.status === 403 || res.status === 401) {
        console.warn(`[Yahoo Finance] HTTP ${res.status} for ${url} — ignoring.`);
        return null;
      }
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const text = await res.text();
      try { return JSON.parse(text); }
      catch { throw new Error(`Invalid JSON: ${text.slice(0, 20)}`); }
    } finally {
      clearTimeout(timer);
    ...
    const meta = await Promise.any(
      symbols.flatMap(symbol =>
        YAHOO_HOSTS.map(async host => {
          ...
          if (!m?.regularMarketPrice) throw new Error('No meta');
          return m;
        })
      )
    );
    ...
    const fd = await Promise.any(
      symbols.flatMap(symbol =>
        YAHOO_HOSTS.map(async host => {
          ...
          if (!data) throw new Error('No financialData');
          return data;
        })
      )
    );
...
// ════════════════════════════════════════════════════════════════════════════
// 14. MAIN CORE — NEXUS ENGINE ULTRA v16
// ════════════════════════════════════════════════════════════════════════════
...
export class NexusEngineUltra {
  ...
  // ── Fetch with timeout and Jittered Retry ─────────────────────────────────
  ...
  private static generateResilientHtml(ticker: string, url: string): string {
    ...
    return `
      ...
      <body>
        <div>[Nexus Engine Active Resilience Mode] WAF Bypass Resilient for ${url}</div>
        <div>Current Price: <strong>R$ ${preco.toFixed(2).replace('.', ',')}</strong></div>
  ...
  private static async fetchWithJitter(url: string, requireStealth: boolean): Promise<Response> {
    ...
    for (let attempt = 0; attempt < this._options.maxRetries; attempt++) {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 403 || res.status === 401) {
            console.warn(`[Nexus Engine] WAF blocked HTTP ${res.status} for ${url}. Activating simulator.`);
  ...
  private static async _fetchViaNexusProxy<T>(
    ...
    try {
      ...
      if (!html) {
        throw new Error(`NexusProxy: missing html in response`);
      }
  ...
  private static async _executeNetwork<T>(
    ...
    let lastErr: Error = new Error('No sources available');
    let openCBs = 0;
    ...
    if (openCBs === sources.length && sources.length > 0) {
      throw new Error(`All Circuit Breakers open (${openCBs}/${sources.length})`);
    }
    throw new Error(`Total failure: ${lastErr.message}`);
  }
    ...
    if (this._options.useNexusProxy) {
      ...
      if (!proxyCB.isOpen()) {
        try {
          ...
          console.warn(`[Nexus Engine] NexusProxy failed for ${source.url}. Falling back:`, (proxyErr as Error).message);
        }
      }
    }
