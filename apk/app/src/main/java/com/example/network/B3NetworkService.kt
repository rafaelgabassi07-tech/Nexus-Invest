...
object B3NetworkService {
        ...
        if (toFetch.isNotEmpty()) {
            ...
            fun acceptMapped(mapped: B3AssetData?) {
                if (mapped?.ticker?.isNotBlank() == true) {
                    val cleanMappedTicker = mapped.ticker.trim().uppercase(Locale.ROOT)
                    val safeMapped = if (assetIsGoodSnapshot(mapped)) {
                        saveBestSnapshot(mapped.copy(ticker = cleanMappedTicker))
                        mapped.copy(ticker = cleanMappedTicker)
                    } else {
                        loadBestSnapshot(cleanMappedTicker)?.copy(
                            proxyStatus = mapped.proxyStatus.ifBlank { "PARTIAL" },
                            isPartial = true,
                            partialDataGuidance = mapped.partialDataGuidance.ifBlank { "Resposta parcial em lote; usando último snapshot bom." },
                            shouldKeepPreviousSnapshot = true,
                            fromLocalSnapshot = true
                        ) ?: mapped.copy(ticker = cleanMappedTicker)
                    }
    ...
    fun fetchProxyDiagnosticsSummary(): ProxyDiagnosticsSummary {
        ...
        val now = System.currentTimeMillis()
        return ProxyDiagnosticsSummary(
